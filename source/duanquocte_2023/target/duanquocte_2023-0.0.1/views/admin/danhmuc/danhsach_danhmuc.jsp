...
<body>
		<form action="<c:url value='/admin_quanly_danhmuc?action=danhsach_danhmuc'/>" id="form_submit" method="get">
			...
			<div class="table-responsive text-nowrap">
				<table class="table table-striped">
					...
					<tbody class="table-border-bottom-0">
						...
						<c:forEach var="item" items="${danhmuc.danhsach}">
							<tr>
								<td><i class="fab fa-angular fa-lg text-danger me-3"></i> <strong>${item.maDanhMuc}
										</strong></td>
												...
												class="bx bx-trash me-1"></i> Delete</a> -->
												 <a
														class="dropdown-item"
														   ...
														   href='<c:url value="/admin_xoa_danhmuc?maDanhMuc=${item.maDanhMuc}&action=xoa_danhmuc"/>'
														onclick="return confirm('Cảnh báo!khi xóa danh mục này dữ liệu về danh mục bạn vừa xóa sẽ mất trên hồ sơ dự án!Bạn muốn tiếp tục ?');"><i
	...
	var currentPage=${nguoidung.page}; */
			var totalPages=${danhmuc.totalPage};
			var currentPage=${danhmuc.page};
			var limit=20;
				$(function() {
					window.pagObj = $('#pagination').twbsPagination({
						...
						onPageClick : function(event, page) {
							if(currentPage!=page)
								{
								$('#action').val('danhsach_danhmuc');
								$('#maxPageItem').val(limit);
								$('#page').val(page);
								$('#sapxeptheothuoctinh').val('madanhmuc');
