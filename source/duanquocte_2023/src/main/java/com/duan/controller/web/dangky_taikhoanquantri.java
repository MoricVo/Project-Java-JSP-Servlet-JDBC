...
/**
 * Servlet implementation class dangky_taikhoanquantri
 */

@WebServlet(urlPatterns = {"/dang-ky-taikhoan-quantri","/dang-ky-xac-thuc-taikhoan-quantri"})
public class dangky_taikhoanquantri extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public dangky_taikhoanquantri() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action != null && action.equals("dangky_taikhoan_quantri")) 
		{
		...
		RequestDispatcher rd=request.getRequestDispatcher("/register_manager.jsp");
		rd.forward(request, response);
		}
		else if (action != null && action.equals("xacthuc_taikhoan_quantri")) 
			{
				try {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.setCharacterEncoding("utf-8");
		String action = request.getParameter("action");
		if (action != null && action.equals("dangky_taikhoan_quantri_post")) 
		{
		...
		int ItrangThai=Integer.parseInt(trangThai);
		...
		if(nguoidung.kiemTraGmail(gmail)) {
			...
			bang_nguoidung_model kh = new bang_nguoidung_model(ma,ImaDonViTrucThuoc,ImaChucVu,hoNguoiDung,tenNguoiDung,gmail,matKhau,quyenHan,ItrangThai);
			if(nguoidung.insert_nguoidung2(kh)>0)
			{
